...
zrtos_vfs_module_mcp2515__ERROR zrtos_vfs_module_mcp2515__reset(void)
{
    startSPI();
...
uint8_t zrtos_vfs_module_mcp2515__readRegister(const REGISTER reg)
{
    startSPI();
...
void zrtos_vfs_module_mcp2515__readRegisters(const REGISTER reg, uint8_t values[], const uint8_t n)
{
    startSPI();
    SPIn->transfer(ZRTOS_VFS_MODULE_MCP2515_INSTRUCTION__READ);
    SPIn->transfer(reg);
...
void zrtos_vfs_module_mcp2515__setRegister(const REGISTER reg, const uint8_t value)
{
    startSPI();
...
void zrtos_vfs_module_mcp2515__setRegisters(const REGISTER reg, const uint8_t values[], const uint8_t n)
{
    startSPI();
    SPIn->transfer(ZRTOS_VFS_MODULE_MCP2515_INSTRUCTION__WRITE);
    SPIn->transfer(reg);
...
void zrtos_vfs_module_mcp2515__modifyRegister(const REGISTER reg, const uint8_t mask, const uint8_t data)
{
    startSPI();
...
uint8_t zrtos_vfs_module_mcp2515__getStatus(void)
{
    startSPI();
...
zrtos_vfs_module_mcp2515__ERROR zrtos_vfs_module_mcp2515__setBitrate(const CAN_SPEED canSpeed)
{
...
bool zrtos_vfs_module_mcp2515__checkReceive(void)
{
...
bool zrtos_vfs_module_mcp2515__checkError(void)
{
...
uint8_t zrtos_vfs_module_mcp2515__getErrorFlags(void)
{
    return readRegister(MCP_EFLG);
}

void zrtos_vfs_module_mcp2515__clearRXnOVRFlags(void)
{
	modifyRegister(MCP_EFLG, EFLG_RX0OVR | EFLG_RX1OVR, 0);
}

uint8_t zrtos_vfs_module_mcp2515__getInterrupts(void)
{
    return readRegister(MCP_CANINTF);
}

void zrtos_vfs_module_mcp2515__clearInterrupts(void)
{
    setRegister(MCP_CANINTF, 0);
}

uint8_t zrtos_vfs_module_mcp2515__getInterruptMask(void)
{
    return readRegister(MCP_CANINTE);
}

void zrtos_vfs_module_mcp2515__clearTXInterrupts(void)
{
    modifyRegister(MCP_CANINTF, (CANINTF_TX0IF | CANINTF_TX1IF | CANINTF_TX2IF), 0);
}

void zrtos_vfs_module_mcp2515__clearRXnOVR(void)
{
	uint8_t eflg = getErrorFlags();
	if (eflg != 0) {
		clearRXnOVRFlags();
...
uint8_t zrtos_vfs_module_mcp2515__errorCountRX(void)                             
{
    return readRegister(MCP_REC);
}

uint8_t zrtos_vfs_module_mcp2515__errorCountTX(void)                             
{
